...
let splashAnimRunning = true;

/* ── 1. SPLASH: 5→1 (no club name), then inaugurate button only ── */
(function initSplash() {
  const splash    = document.getElementById('splash');
  const splashNum = document.getElementById('splashNum');
  const ring      = document.getElementById('splashRing');
  const cdPhase   = document.getElementById('splash-countdown-phase');
  const inPhase   = document.getElementById('splash-inaugurate-phase');

  // Splash particles
  const sc   = document.getElementById('splash-canvas');
  const sCtx = sc.getContext('2d');
  sc.width = window.innerWidth; sc.height = window.innerHeight;
  const SCOLS = ['rgba(200,169,110,','rgba(184,196,212,','rgba(232,213,163,'];
  const sP = Array.from({length:55},()=>({
    x:Math.random()*sc.width, y:Math.random()*sc.height,
    r:.5+Math.random()*2, vx:(Math.random()-.5)*.4, vy:-(0.2+Math.random()*.6),
    col:SCOLS[Math.floor(Math.random()*SCOLS.length)], op:.1+Math.random()*.35
  }));
  function drawSP(){
    if(!splashAnimRunning)return;
    sCtx.clearRect(0,0,sc.width,sc.height);
    sP.forEach((p,i)=>{
      p.x+=p.vx; p.y+=p.vy;
      if(p.y<-10){sP[i].y=sc.height+5;sP[i].x=Math.random()*sc.width;}
      sCtx.beginPath();sCtx.arc(p.x,p.y,p.r,0,Math.PI*2);
      sCtx.fillStyle=p.col+p.op+')';sCtx.shadowBlur=4;sCtx.shadowColor=p.col+'.5)';sCtx.fill();
    });
    requestAnimationFrame(drawSP);
  }
  drawSP();

  /* Circumference for r=80 circle: 2π×80 ≈ 502.65 */
  const CIRC = 2 * Math.PI * 80;
  let count = 5;

  function setRing(frac){ ring.style.strokeDashoffset = CIRC*(1-frac); }

  function step(){
    /* update number with pop animation */
    splashNum.style.animation='none';
    void splashNum.offsetWidth;
    splashNum.textContent = count;
    splashNum.style.animation='numPop .4s cubic-bezier(.34,1.56,.64,1) both';

    /* drain ring from full to empty over 1 second */
    const t0 = performance.now();
    function drain(now){
      const frac = Math.max(0, 1-(now-t0)/1000);
      setRing(frac);
      if(frac>0){ requestAnimationFrame(drain); }
      else {
        count--;
        if(count>0){ step(); }
        else {
          /* Switch to inaugurate phase — NO club name here */
          cdPhase.style.display='none';
          inPhase.style.display='flex';
        }
      }
    }
    setRing(1);
...
function launchWebsite(){
  ...
  setTimeout(()=>{ splash.style.display='none'; splashAnimRunning=false; }, 900);
  fireConfetti();
}
